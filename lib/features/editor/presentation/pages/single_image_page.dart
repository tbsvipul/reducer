...
class _SingleImageScreenState extends ConsumerState<SingleImageScreen>
  ...
  void initState() {
    ...
    _tabController.addListener(() {
      if (!_tabController.indexIsChanging && mounted) {
        ref.read(singleImageTabIndexProvider.notifier).state =
  ...
  Future<void> _handleProcess() async {
    await AdManager().showInterstitialAd(
      onComplete: () async {
        await ref
            .read(singleImageControllerProvider.notifier)
            .processFinalImage();
        if (mounted) {
  ...
  Future<void> _saveToGallery(SingleImageState state) async {
    ...
    await AdManager().showInterstitialAd(
      onComplete: () async {
        try {
          if (!mounted) return;
          final ok = await PermissionService.instance.ensurePhotosPermission(
            context,
          );
          ...
          final processedRelativePath =
              'history/proc_$timestampMs.${state.settings.format.extension}';
  ...
  Future<void> _shareImage(SingleImageState state) async {
    ...
    try {
      ...
      final file = File(
        '${tempDir.path}/share_${DateTime.now().millisecondsSinceEpoch}.${state.settings.format.extension}',
      );
      await file.writeAsBytes(state.processedImageBytes!);

      if (mounted) {
        await SharePlus.instance.share(
          ShareParams(files: [XFile(file.path)], text: l10n.shareWithReducer),
        );
      }
    } catch (e) {
  ...
  Widget _buildPreviewHeader(
    SingleImageState state,
    bool isDark,
    AppLocalizations l10n,
  ) {
    final displayImage = _showOriginal
        ? state.originalThumbnail!
        : (state.previewThumbnail ?? state.originalThumbnail!);
              ...
              Stack(
                children: [
                  ...
                  Positioned(
                    ...
                    child: Semantics(
                      button: true,
                      label: _showOriginal ? l10n.showAfter : l10n.showBefore,
                      child: Material(
    ...
    return Consumer(
      builder: (context, ref, child) {
        ...
        final isProcessing = ref.watch(
          singleImageControllerProvider.select((s) => s.isProcessingFinal),
        );
                    ...
                    child: AppButton(
                      ...
                      onPressed: () async {
                        ...
                        try {
                          await ref
                              .read(singleImageControllerProvider.notifier)
                              .pickImage(ImageSource.gallery, l10nCapture);
                        } catch (e) {
                    ...
                    child: AppButton(
                      ...
                      onPressed: () async {
                        ...
                        try {
                          await ref
                              .read(singleImageControllerProvider.notifier)
                              .pickImage(ImageSource.camera, l10nCapture);
                        } catch (e) {
                          if (e is ValidationResult && context.mounted) {
                            ImageValidator.showValidationDialog(context, e);
                          }
